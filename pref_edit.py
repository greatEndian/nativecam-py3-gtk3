...
class PrefEditor():
    ...
    def __init__(self, ncam, is_metric, catalog, path, pre_amble, post_amble, sysdir) :
        ...
        _lbl = builder.get_object("valueLbl")
        _css = gtk.CssProvider()
        _css.load_from_data(b"label { font: bold 16px Sans; }")
        _lbl.get_style_context().add_provider(_css, gtk.STYLE_PROVIDER_PRIORITY_APPLICATION)
    ...
    def save_click(self, *args):
        ...
        self.config_pref.set('display', 'width', str(self.getint(self.adj_WindowWidth.get_value())))
        self.config_pref.set('display', 'name-ellipsis', str(self.ellip_name_combo.get_active()))
        self.config_pref.set('display', 'name_col_width', str(self.getint(self.adj_nameColWidth.get_value())))
        self.config_pref.set('display', 'master_tv_width', str(self.getint(self.adj_tvWidth.get_value())))
        self.config_pref.set('display', 'restore_expand_state', str(self.restore_tvstate.get_active()))
        self.config_pref.set('display', 'tv2_expandable', str(self.tv2_expandable.get_active()))
        self.config_pref.set('display', 'tv_expandable', str(self.tv_expandable.get_active()))

        if not self.config_pref.has_section('icons_size') :
            self.config_pref.add_section('icons_size')
        self.config_pref.set('icons_size', 'treeview', str(self.getint(self.adj_tviconsize.get_value())))
        self.config_pref.set('icons_size', 'add_menu', str(self.getint(self.adj_addmenuiconsize.get_value())))
        self.config_pref.set('icons_size', 'menu', str(self.getint(self.adj_menuiconsize.get_value())))
        self.config_pref.set('icons_size', 'toolbar', str(self.getint(self.adj_tbIconSize.get_value())))
        self.config_pref.set('icons_size', 'add_dlg', str(self.getint(self.adj_adddlgimgsize.get_value())))
        self.config_pref.set('icons_size', 'ncam_toolbar', str(self.getint(self.adj_histiconsize.get_value())))

        if not self.config_pref.has_section('virtual_kb') :
            self.config_pref.add_section('virtual_kb')
        self.config_pref.set('virtual_kb', 'minimum_width', str(self.getint(self.adj_vkbwidth.get_value())))
        ...
        self.config_def.set('general', 'time_out', str(self.adj_timeout_value.get_value()))
        self.config_def.set('general', 'digits', str(int(self.digits_combo.get_active()) + 1))
        self.config_def.set('general', 'default_metric', str(int(self.units_combo.get_active())))
        if self.catalog in ['mill', 'plasma'] :
            self.config_def.set('general', 'show_final_cut', str(self.finalcut_chk.get_active()))
            self.config_def.set('general', 'show_bottom_cut', str(self.finalbottom_chk.get_active()))
        self.config_def.set('general', 'gmoccapy_time_out', str(self.adj_gmoccapy.get_value()))
        self.config_def.set('general', 'autosave', str(self.autosave_chk.get_active()))

        if not self.config_def.has_section('ngc') :
            self.config_def.add_section('ngc')
        self.config_def.set('ngc', 'init_str', str(self.entryInit.get_text()))
        self.config_def.set('ngc', 'post_amble', str(self.entryPost.get_text()))
        self.config_def.set('ngc', 'use_pct_signs', str(self.use_pct_signs_chk.get_active()))
        if self.catalog == 'mill' :
            self.config_def.set('ngc', 'off_rot_coord_system', str(self.comboCoords.get_active()))
            self.config_def.set('ngc', 'spindle_all_time', str(self.spindle_always_chk.get_active()))
        if self.catalog in ['mill', 'lathe'] :
            self.config_def.set('ngc', 'spindle_acc_time', str(self.adj_spindledelay.get_value()))

        if self.catalog == 'mill' :
            if not self.config_def.has_section('probe') :
                self.config_def.add_section('probe')
            self.config_def.set('probe', 'probe_func', str(self.comboProbe.get_active() + 2))
            self.config_def.set('probe', 'probe_tool_len_comp', str(self.tlo_chk.get_active()))
            ...
            self.config_def.set(probe_section, 'probe_feed', str(self.adj_probefeed.get_value()))
            self.config_def.set(probe_section, 'probe_latch', str(self.adj_probelatch.get_value()))
            self.config_def.set(probe_section, 'probe_latch_feed', str(self.adj_probelatchfeed.get_value()))
            self.config_def.set(probe_section, 'probe_tip_dia', str(self.adj_probedia.get_value()))
            self.config_def.set(probe_section, 'probe_safe', str(self.adj_probesafe.get_value()))
            self.config_def.set(probe_section, 'probe_height', str(self.adj_probeheight.get_value()))

            self.config_def.set(drill_section, 'center_drill_depth', str(self.adj_centerdrill_depth.get_value()))

            if not self.config_def.has_section('pocket') :
                self.config_def.add_section('pocket')
            self.config_def.set('pocket', 'mode', str(self.combo_pck.get_active()))

            if not self.config_def.has_section('optimizing') :
                self.config_def.add_section('optimizing')
            self.config_def.set('optimizing', 'engagement1', str(self.adj_opt_eng1.get_value()))
            self.config_def.set('optimizing', 'engagement2', str(self.adj_opt_eng2.get_value()))
            self.config_def.set('optimizing', 'engagement3', str(self.adj_opt_eng3.get_value()))

            self.config_def.set('optimizing', 'feedfactor1', str(self.adj_opt_ff1.get_value()))
            self.config_def.set('optimizing', 'feedfactor2', str(self.adj_opt_ff2.get_value()))
            self.config_def.set('optimizing', 'feedfactor3', str(self.adj_opt_ff3.get_value()))
            self.config_def.set('optimizing', 'feedfactor4', str(self.adj_opt_ff4.get_value()))
            self.config_def.set('optimizing', 'feedfactor0', str(self.adj_opt_ff0.get_value()))

            self.config_def.set('optimizing', 'speedfactor1', str(self.adj_opt_sf1.get_value()))
            self.config_def.set('optimizing', 'speedfactor2', str(self.adj_opt_sf2.get_value()))
            self.config_def.set('optimizing', 'speedfactor3', str(self.adj_opt_sf3.get_value()))
            self.config_def.set('optimizing', 'speedfactor4', str(self.adj_opt_sf4.get_value()))
            self.config_def.set('optimizing', 'speedfactor0', str(self.adj_opt_sf0.get_value()))
